  ...
  IF TG_OP IN ('INSERT') THEN
    ...
    IF (record_number > 0) THEN
      ...
      if old_record.firedate>NEW.firedate then
        ...
        --oggetto_cambi := ('date_previous',{old_record.firedate},'date_last',{new.firedate});
      end if;
      if old_record.area_ha>NEW.area_ha then
        ...
        --RAISE NOTICE 'JSON costruito %',oggetto_json;
        --oggetto_cambi := ('area_previous',{old_record.area_ha},'area_last',{new.area_ha});
      end if;

      oggetto_json := json_build_array(date_json,area_json);
      RAISE NOTICE 'array JSON costruito %',oggetto_json;
