...
SELECT ba_id,firedate FROM effis.current_burnt_area_evolution ORDER BY ba_id;

SELECT ID,firedate FROM RDAPRD.current_firesevolution ORDER BY ID;

SELECT *
FROM effis.current_burnt_area_evolution a 
     FULL JOIN rdaprd.current_firesevolution b ON a.ba_id = b.id
WHERE a.ba_id IS NULL 
OR b.id IS NULL;
