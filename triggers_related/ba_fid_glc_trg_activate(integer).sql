-- Function: effis.ba_fid_glc_trg_activate(integer)

-- DROP FUNCTION effis.ba_fid_glc_trg_activate(integer);

CREATE OR REPLACE FUNCTION effis.ba_fid_glc_trg_activate(IN fire_id integer)
     ...
     values_area AS(SELECT (stats).*,rastpixwidth,rastpixheight
			 FROM (SELECT ST_ValueCount(ST_Clip(rast, geom_proj)) AS stats,
				      ST_ScaleX(rast) As rastpixwidth, 
		               ...
		               FROM rst.clc_1x1k
				  INNER JOIN burnt_area ON ST_Intersects(geom_proj, rast)) AS foo)
...
END; $BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION effis.ba_fid_glc_trg_activate(integer)
  OWNER TO postgres;
