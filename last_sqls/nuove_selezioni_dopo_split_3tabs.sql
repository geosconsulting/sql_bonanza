...
WHERE count > 0
   ...
   SELECT --ST_AsText(ba.geom), 
       ba.id as "ID", ba.fire AS "Fire ID", 
       ba.firedate as "Fire Date", lc.id ,
       lc.conifer as "Conifer", 
       lc.broadlea as "Broad Leaf", 
       lc.mixed as "Mixed", lo.id, lo.countryful
FROM effis.burnt_area_spatial ba, effis.burnt_area_landcover lc, effis.burnt_area_location lo
WHERE ba.fire = lc.id AND ba.fire = lo.id LIMIT 25;


SELECT ST_AsText(ba.geom,2), 
       ba.id as "ID", ba.fire AS "Fire ID", 
       ba.firedate as "Fire Date", lc.id ,
       lc.conifer as "Conifer", 
       lc.broadlea as "Broad Leaf", 
       lc.mixed as "Mixed", lo.id, lo.countryful
FROM effis.burnt_area_spatial ba, effis.burnt_area_landcover lc, effis.burnt_area_location lo
WHERE ba.fire = lc.id AND lo.country = 'IT' AND ba.fire = lo.id ORDER BY "Conifer" DESC LIMIT 25;